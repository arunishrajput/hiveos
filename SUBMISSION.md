...
- The board can **wear a different world** — Paper Office, Night Watch, Enchanted Forest, Reef
  Station, Alien Colony or Cloud City — swapped from the title bar with no reload. Same sockets,
  same scheduler, same rows; re-dressed. A world may never change what anything *means*, so each
  one's state hues are re-tuned for its own surfaces and **measured at ≥4.5:1 against every
  surface it paints**, and never reassigned
